...
from documentos.serializers import doc_analisis_Serializer1, doc_pruebasU_Serializer, doc_requisitos_Serializer, doc_registroR_Serializer, doc_Preubas_Serializer
...
class DocPruebasUniView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            response['Content-Disposition'] = f'attachment; filename={pdf_data[0]}'
            return HttpResponse(response, content_type='application/pdf', status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST), print(serializer.errors)
...
class DocRegistroRastreoView(APIView):
    permission_classes = [permissions.AllowAny]
    ...
    def save_pdf(self, data):
        ...
        ["Responsable: ", data.get('puesto', None)],
        ["Fecha: ", fecha],
        ]
       
        pdf = PDF(data.get('usuario', None))

        pdf.add_title("Documento de registro de rastreo")
        pdf.add_subtitle("Proceso "+ data.get('nombre_proceso', None))
        pdf.add_table(doc_data)
        pdf.add_text('Objetivo: '+ data.get('objetivo', None))
        pdf.add_text('Indicadores y métricas: \n ' + data.get('ind_met', None))
        ...
        pdf.add_table(ev_data)
        pdf.add_text('Herraienta: '+ data.get('herramienta', None))
        pdf.add_text('Link: '+ data.get('link', None))
        pdf.add_image(base_dir2+data.get('imagen', None))

        pdf_name = f"{data.get('identificador', None)}_{fecha}.pdf"
        pdf_path = base_dir2+f"/assets/archivosPDF/registro_rastreo/{pdf_name}"
        pdf_data = []
    ...
    def post(self, request):
        serializer = doc_registroR_Serializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            pdf_data = self.save_pdf(serializer.data)
            response = FileResponse(open(pdf_data[1], 'rb'))
...
class DocPruebasView(APIView):
    ...
    def save_pdf(self, data):
        ...
        pdf.add_title("Documento de pruebas")
        pdf.add_subtitle("Proceso "+ data.get('nombre_proceso', None))
        pdf.add_text('Objetivo: '+ data.get('objetivo', None))
        pdf.add_text('Identificador: '+ data.get('identificador', None))
        pdf.add_text('Indicadores y métricas: \n ' + data.get('ind_met', None))
        pdf.add_text('Descripción de actividades: \n ' + data.get('des_actividades', None))
        pdf.add_image(base_dir2+data.get('diagrama_flujo', None))
        pdf.add_text('Categoria: '+ data.get('categoria', None))
        ...
        pdf.add_text('Tipo de errores: '+ data.get('tipo_errores', None))
        pdf.add_text('Criterios de aceptación:')
        pdf.add_image(base_dir2+data.get('criterios_aceptacion', None))
        pdf.add_text('Criterios para pruebas de usabilidad: \n'+ data.get('criterios_usabilidad', None))
        pdf.add_image(base_dir2+data.get('criterios_usa_img', None))
        pdf.add_text('Caso de prueba: \n'+ data.get('caso_prueba', None))
        pdf.add_text('Folio: '+ data.get('folio_caso', None))
        pdf.add_image(base_dir2+data.get('caso_prueba_img', None))
        pdf.add_text('Reporte de pruebas: \n'+ data.get('reporte_prueba', None))
        pdf.add_text('Folio: '+ data.get('reporte_folio', None))
        pdf.add_image(base_dir2+data.get('reporte_prueba_img', None))
        pdf.add_text('Metricas de pruebas: \n'+ data.get('metricas', None))
        pdf.add_text('Conclusiones: \n'+ data.get('conclusiones', None))

        pdf_name = f"{data.get('identificador', None)}_{fecha}.pdf"
        pdf_path = base_dir2+f"/assets/archivosPDF/pruebas/{pdf_name}"
        pdf_data = []
        pdf_data.append(pdf_name)
        pdf_data.append(pdf_path)
    ...
    def post(self, request):
        serializer = doc_Preubas_Serializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            pdf_data = self.save_pdf(serializer.data)
            response = FileResponse(open(pdf_data[1], 'rb'))
